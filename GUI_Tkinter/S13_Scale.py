...
It provides an alternative to the Entry widget when the user is forced to select
...
It is shown in the top left corner if the scale is horizontal or the top right corner if the scale is vertical.
13	length	It represents the length of the widget.
It represents the X dimension if the scale is horizontal or y dimension if the scale is vertical.
...
16	repeatdelay	This option tells the duration up to which the button is to be pressed
...
21	takefocus	The focus cycles through the scale widgets by default.
We can set this option to 0 if we don't want this to happen.
